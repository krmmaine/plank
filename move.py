def move(cell, time, stay, left, right, up, random, yPos, xPos, occupied, fib, vegf, pro, movement, file, T, xSteps, ySteps):
    def backtrack(x, y, xSteps, ySteps):
        print("backtrack")
        ...
        if yPos[cell][time + 1] > 0:                        # need time + 1 because the cell just moved down so it is no longer in the capillary
            ...
            if len(movement) > 1:
                if movement[len(movement)-2] == 1:
                    backtrack(xPos[cell][time+1], yPos[cell][time+1], xSteps, ySteps)
